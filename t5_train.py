...
training_args = TrainingArguments(
    ...
    save_total_limit=1,
    save_strategy="epoch",
    report_to="none"  # Disable reporting to wandb or other tracking services
)
...
for ref, pred in zip(actual_interpretations, generated_interpretations):
    ...
    rouge2 += scores["rouge2"].fmeasure
    rougeL += scores["rougeL"].fmeasure
n = len(actual_interpretations)
print(f"ROUGE-1: {rouge1/n:.4f}, ROUGE-2: {rouge2/n:.4f}, ROUGE-L: {rougeL/n:.4f}")

# Perplexity
perplexity = calculate_perplexity(generated_interpretations, tokenizer)
print(f"Perplexity: {perplexity:.4f}")

# BERTScore
bert_score_result = bert_score.compute(predictions=generated_interpretations, references=actual_interpretations, lang="en")
bert_avg = np.mean(bert_score_result["f1"])
print(f"BERTScore (F1): {bert_avg:.4f}")
...
filename = "/home/linuxu/Desktop/NLP_project/t5_5_epochs/dream_llm/evaluation_metrics_val.json"
with open(filename, "w") as f:
    json.dump(test_results, f, indent=4)

print(f"Test evaluation metrics saved to {filename}")